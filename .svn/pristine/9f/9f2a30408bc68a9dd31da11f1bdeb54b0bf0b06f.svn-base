    ...
    $.jqmCalendar = function (element, options) {
        ...
        function init() {
            ...
            $previous = $("<a href='#' data-role='button' data-icon='arrow-l' data-iconpos='notext' class='previous-btn'>Previous</a>").click(function (event) {
            	//alert("previous button clicked- do ajax call to get the details of year "+ plugin.settings.date.getFullYear() +", month"+ (plugin.settings.date.getMonth() - 1));
            ...
            $previous = $("<a href='#' data-role='button' data-icon='arrow-r' data-iconpos='notext' class='next-btn'>Next</a>").click(function (event) {
            	//alert("next button clicked- do ajax call to get the details of year "+ plugin.settings.date.getFullYear() +", month"+ (plugin.settings.date.getMonth() + 1));
                refresh(new Date(plugin.settings.date.getFullYear(), plugin.settings.date.getMonth() + 1, plugin.settings.date.getDate()));
            }).appendTo($th);
        ...
        function addCell($row, date, hidden, selected, event) {
        ...
        function refresh(date) {
            ...
            $element.bind('change', function (event, begin) {
                ...
                // Find events for this date
                //Commenting this to remove the event and add the legend
                ...
                $("<li class='absent'>Absent</li>").appendTo($listview);
                $("<li class='tardy'>Tardy</li>").appendTo($listview);
